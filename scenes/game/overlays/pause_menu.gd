...
@onready var pause_music: AudioStreamPlayer = $"pause_music"
...
func _ready() -> void:
	...
	pause_music.stream.loop = true # yeah.
	pause_music.volume_linear = 0.000001
	Global.request_audio_fade(pause_music, 0.7, 5.0)
	pause_music.play(randf_range(mlength * 0.25, mlength))
